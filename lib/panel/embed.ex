defmodule Grafana.Panel.Embed do
  ...
  defp build_src(uri, panel_id, from, to, endpoint, probe, outliers) do
    "http://#{@username}:#{@password}@#{server_url}/dashboard-solo/#{uri}?panelId=#{panel_id}&from=#{from}&to=#{to}&var-endpoint=#{endpoint}&var-probe=#{probe}&var-outliers=#{outliers}"
  end
